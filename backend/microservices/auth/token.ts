...
export async function testToken(jwt: string) {
	if (!jwt) return false;

	try {
		const decode = (await jwtToken.verify(jwt, config.get("jwtSecret"))) as IdecodeJWT;
		...
		const token = await authDB.getJWT(userId);

		if (!token || !token.length) return false;

		for (let i = 0; i < token.length; i++) {
			if (token[i].token === jwt) {
				decode.userId = normalize.deleteSpace(decode.userId);
